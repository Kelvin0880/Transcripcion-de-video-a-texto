...
interface TranscriptionResult {
  ...
  duration: number
  summary?: string
  highlights?: Array<{ start: number; end: number; text: string; importance: number }>
  sentiment?: string
...
export default function Home() {
  ...
  const handleFileSelect = useCallback((selectedFile: File) => {
    // Validar tipo de archivo (video y audio)
    const isVideo = selectedFile.type.startsWith('video/')
    const isAudio = selectedFile.type.startsWith('audio/')
    
    if (!isVideo && !isAudio) {
      toast.error('Por favor selecciona un archivo de video o audio válido')
      return
    }
    ...
    setError(null)
    setResult(null)
    toast.success(`${isVideo ? 'Video' : 'Audio'} cargado correctamente`)
  }, [])
  ...
  const handleTranscription = useCallback(async () => {
    ...
    try {
      ...
      formData.append('file', file) // Corregido: cambiar 'video' por 'file'
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error al procesar el archivo. Inténtalo de nuevo.'
  ...
  const handleAnalysis = useCallback(async (type: string) => {
    if (!result?.text) {
      toast.error('Necesitas una transcripción primero');
      return;
    }
    ...
    try {
      ...
      toast.success(`¡${type === 'summary' ? 'Resumen' : type === 'highlights' ? 'Highlights' : 'Análisis'} completado!`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Error en el análisis';
      toast.error(errorMessage);
    } finally {
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-xl font-semibold mb-2">
                    {isDragging ? '¡Suelta tu archivo aquí!' : 'Arrastra tu video o audio aquí'}
                  </p>
                  ...
                  <div className="mt-4 text-sm text-gray-400">
                    <p>Video: MP4, AVI, MOV, MKV | Audio: MP3, WAV, FLAC, M4A</p>
              ...
              {result && (
                <div className="flex flex-wrap gap-2 mt-4">
                  <motion.button
                    ...
                    className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 disabled:from-gray-600 disabled:to-gray-700 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 shadow-lg hover:shadow-xl disabled:cursor-not-allowed"
                  >
                  ...
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => handleAnalysis('highlights')}
                    disabled={isAnalyzing || !result.timestamps?.length}
                    className="bg-gradient-to-r from-pink-500 to-red-500 hover:from-pink-600 hover:to-red-600 disabled:from-gray-600 disabled:to-gray-700 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 shadow-lg hover:shadow-xl disabled:cursor-not-allowed"
                  >
                    {isAnalyzing && activeAnalysis === 'highlights' ? (
                      <>
                        <FaSpinner className="animate-spin mr-1" />
                        Analizando...
                      </>
                  ...
                  <motion.button
                    ...
                    disabled={isAnalyzing}
                    className="bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 disabled:from-gray-600 disabled:to-gray-700 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 shadow-lg hover:shadow-xl disabled:cursor-not-allowed"
                  >
                    {isAnalyzing && activeAnalysis === 'sentiment' ? (
                      <>
                        <FaSpinner className="animate-spin mr-1" />
                        Analizando...
                      </>
                  ...
                  <div className="p-4 bg-white/5 rounded-xl">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                      {result.highlights.map((highlight, index) => (
                        <div
                          key={index}
                          className="flex items-start gap-3 p-3 bg-gradient-to-r from-pink-500/10 to-red-500/10 rounded-lg border border-pink-500/20"
                        >
                          <div className="flex flex-col items-center">
                            ...
                            <div className="flex mt-1">
                              {Array.from({ length: highlight.importance || 1 }).map((_, i) => (
                        ...
                        <div>
                          <p className="text-sm text-gray-200 capitalize">
                            {result.sentiment === 'positive' ? 'Positivo' : 
                             result.sentiment === 'negative' ? 'Negativo' : 'Neutral'}
                          </p>
                          <p className="text-xs text-gray-400">
                            Confianza: {result.sentimentConfidence ? `${Math.round(result.sentimentConfidence * 100)}%` : 'N/A'}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div>
              ...
              <div className="text-gray-400 space-y-2">
                ...
                <div className="flex gap-4 mt-4">
                  ...
                  <motion.a
                    whileHover={{ scale: 1.1 }}
                    href="https://github.com/Kelvin0880"
                    target="_blank"
                  ...
                  <motion.a
                    whileHover={{ scale: 1.1 }}
                    href="https://www.linkedin.com/in/kelvin-jose-pi%C3%B1a-9a5249373/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-blue-400 hover:text-blue-300"
                  >
